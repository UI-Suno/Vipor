...
local Purchase = ToolsSettings:Dropdown("Purchase", {"gravity", "speed", "fusion", "trowel", "hourglass", "hook"}, function(Dro)
	local ohTable1 = {
		["kind"] = "gear",
		["item"] = Dro,
		["method"] = "regular"
	...
	game:GetService("ReplicatedStorage").buy:InvokeServer(ohTable1)
end)
...
coroutine.wrap(function()
	while true do
		...
		task.wait(0.5)
		table.remove(Stage, 2)
		table.remove(Stage, 3)
		table.remove(Stage, 4)
		table.remove(Stage, 5)
		table.remove(Stage, 6)
		task.wait(2)
	end
end)()
local StageTP = StageSettings:Dropdown("Stage", Stage, function(Dro)
	for i,j in pairs(Dro:GetChildren()) do
